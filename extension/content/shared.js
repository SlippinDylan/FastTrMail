(() => {
  ...
  const i18nApi = globalThis.FastTrMailI18n;
  ...
  app.state = {
    ...
    uiLanguage: "auto",
    uiLocale: i18nApi?.resolveUiLanguage?.("auto") || "en",
    uiLocalePromise: null
  };
  ...
  app.i18n = {
    runtime: i18nApi,
    async refreshLocale() {
      if (app.state.uiLocalePromise) {
        return app.state.uiLocalePromise;
      }
      ...
      app.state.uiLocalePromise = (async () => {
        ...
        return app.state.uiLocale;
      })();

      try {
        return await app.state.uiLocalePromise;
      } finally {
        app.state.uiLocalePromise = null;
      }
    },
    getLocale() {
      return app.state.uiLocale || i18nApi.resolveUiLanguage(app.state.uiLanguage || "auto");
    },
    t(key, replacements, locale = app.i18n.getLocale()) {
      return i18nApi.t(locale, key, replacements);
    },
    getErrorMessage(errorCode, locale = app.i18n.getLocale()) {
      return i18nApi.getErrorMessage(locale, errorCode);
    },
    resolveErrorMessage(response, locale = app.i18n.getLocale()) {
      const metadataMessage = typeof response?.metadata?.upstreamMessage === "string"
        ? response.metadata.upstreamMessage.trim()
        : "";

      if (response?.errorCode) {
        const localizedMessage = i18nApi.getErrorMessage(locale, response.errorCode);
        return metadataMessage
          ? `${localizedMessage}\n${metadataMessage}`
          : localizedMessage;
      }

      return i18nApi.getErrorMessage(locale, i18nApi.ERROR_CODES.TRANSLATION_REQUEST_FAILED);
    }
  };
})();
